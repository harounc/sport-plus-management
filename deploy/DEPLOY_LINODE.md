# Déploiement Linode — harounc.com

Prévu pour Ubuntu 24.04, Nginx, PostgreSQL, Gunicorn et Certbot. Le DNS `harounc.com` pointe déjà vers `172.104.159.89`.
...
sudo cp deploy/nginx-harounc.com.conf /etc/nginx/sites-available/harounc.com
sudo ln -s /etc/nginx/sites-available/harounc.com /etc/nginx/sites-enabled/harounc.com
sudo nginx -t
...
```bash
sudo certbot --nginx -d harounc.com --redirect
sudo certbot renew --dry-run
```

Le certificat actuellement présenté par le serveur ne correspond pas à `harounc.com`; Certbot doit donc être exécuté avant la mise en ligne finale.
